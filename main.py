...
@register("sunset_forecast", "ChanGR", "晚霞与云海预报", "1.0.0")
class SunsetForecastPlugin(Star):
    def __init__(self, context: Context, config=None):
        super().__init__(context)
        self.config = config
        logger.info("插件 [sunset_forecast] 已加载。")
    ...
    async def _reply_sunset(self, event: AstrMessageEvent):
        ...
        if resolve_spot(location):
            ...
            return
        yield event.plain_result(f"正在查询 {location} 晚霞…")
        try:
            days = await asyncio.to_thread(forecast_location, location, days=self._days())
    ...
    async def cmd_sunset(self, event: AstrMessageEvent):
        ...
        async for item in self._reply_sunset(event):
            yield item
    ...
    async def cmd_cloudsea(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(f"正在查询 {spot} 云海…")
        try:
    ...
    async def cmd_both(self, event: AstrMessageEvent):
        ...
        yield event.plain_result(f"正在查询 {city} 晚霞 + {spot} 云海…")
        try:
            sunset_days, sea_days = await asyncio.gather(
                asyncio.to_thread(forecast_location, city, days=self._days()),
                asyncio.to_thread(forecast_cloud_sea, spot, days=self._days()),
            )
        except ForecastError as exc:
    ...
    async def tool_cloudsea(self, event: AstrMessageEvent, spot: str) -> str:
        ...
        try:
            days = await asyncio.to_thread(forecast_cloud_sea, name, days=self._days())
        except Exception as exc:
            return f"云海查询失败：{exc}"
